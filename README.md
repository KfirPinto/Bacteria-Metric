...
- `embeddings_labels.npy`: Numpy file containing bacterial names corresponding to the embeddings

#### Generate Required Files Using the Processing Script

Use the provided `process_embedding_labels.py` script to convert and format your raw data:

**Generate all required files at once:**
```bash
python process_embedding_labels.py all --npy-input embeddings_labels.npy --csv-output embeddings_labels.csv --taxonomy-input filtered_bacteria.csv --detailed-output bacterial_lineage.csv --species-output species_genus.csv
```

**Or run operations separately:**

1. **Convert embeddings labels from numpy to CSV:**
```bash
python process_embedding_labels.py convert --input embeddings_labels.npy --output embeddings_labels.csv
```

2. **Process taxonomy data:**
```bash
python process_embedding_labels.py taxonomy --input filtered_bacteria.csv --detailed-output bacterial_lineage.csv --species-output species_genus.csv
```

#### Final Required Files
After running the processing script, you should have:
- `embeddings.npy`: Pre-computed bacterial embeddings (numpy array) - **your original file**
- `species_genus.csv`: CSV file containing bacterial names (only genus and species) corresponding to the embeddings - **generated by processing script**
**this file will be the labels of the embeddings for the taxonomy_evaluation script**
- `bacterial_lineage.csv`: CSV file containing full formatted taxonomy information for each bacteria - **generated by processing script**
**this file will be the taxonomy information of the embeddings for the taxonomy_evaluation script**
- (Optional) embedding_variances.npy or .csv: A file containing one variance value per embedding — used to scale dot size in taxonomy plots

You can either place these files in the same directory as the clustering analysis script or specify their paths using command-line arguments.
