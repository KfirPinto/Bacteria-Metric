...
def vectorize_sample_constituency_loss(H):
    ...
    j_idx, k_idx = torch.triu_indices(d, d, offset=1)
    H_j = H[:, j_idx, :]
    H_k = H[:, k_idx, :]
    other_H_k = other_H[:, k_idx, :]
    
    # Fixed normalization with eps
...
def vectorize_bacteria_constituency_loss(H):
    ...
    j_idx, k_idx = torch.triu_indices(n, n, offset=1)
    H_j = H[j_idx, torch.arange(d).unsqueeze(1)]
    H_k = H[k_idx, torch.arange(d).unsqueeze(1)]
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            if torch.isnan(total_loss):
                print(f"⚠️ NaN detected at epoch {epoch}. Stopping trial early.")
